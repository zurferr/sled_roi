...
total_assets = st.select_slider("Number of analytics assets (Snowflake tables, columns, BI dashboards, ...)", options=options, value=options[3])

penalty = min(total_assets / 1e7, 0.1) + 0.1
...
with cola:
    ...
    productivity_value = round(num_people * salary * penalty)
    onboarding_value = round(num_people * salary * 1/retention * 1/12)

    st.metric('More effective data team', "{:,}$".format(productivity_value + onboarding_value), delta=None, delta_color="normal", help=f'On average data professionals save {round(penalty * 40, 1)} hours / week ({round(penalty * 100)}%) because of faster troubleshooting, better change impact analysis and less duplicate and manual work. New data professionals save at least 1 month of onboarding time.')
...
with colb:
    ...
    perc_decision_m = st.slider("% of decision makers working on Snowflake data", 0.0, 1.0, 0.3)
    num_dq_incidents = st.slider(" days per month with data quality incidents", 1, 30, 5)

    """
    #### Cost of Bad Data
    """
    bad_data_cost = round(revenue * perc_decision_m * num_dq_incidents / 30 * penalty)
    st.metric('Business opportunity cost', "{:,}$".format(bad_data_cost), delta=None, delta_color="normal", help=f'The business opportunity cost is the revenue that is lost because of bad data. Decision based on bad data are typically {round(penalty * 100)}% less effective, e.g. inefficiently allocated marketing budget.')
...
## Investment
"""
price_year = round(math.sqrt(total_assets) * 10) * 12
...
st.write(f" *More info? hi@sled.so*")
